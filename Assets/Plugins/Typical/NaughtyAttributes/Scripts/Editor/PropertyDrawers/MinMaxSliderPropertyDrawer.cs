	...
	public class MinMaxSliderPropertyDrawer : PropertyDrawerBase
	{
		...
		protected override void OnGUI_Internal(Rect rect, SerializedProperty property, GUIContent label)
		{
			...
			if (property.propertyType == SerializedPropertyType.Vector2 || property.propertyType == SerializedPropertyType.Vector2Int)
			{
				...
				void MinMaxSlider(ref float  xValue, Rect sliderRect, ref float yValue)
				{
					(float x, float y) old = (xValue, yValue);
					EditorGUI.MinMaxSlider(sliderRect, ref xValue, ref yValue, minMaxSliderAttribute.MinValue, minMaxSliderAttribute.MaxValue);
					if (old.x != xValue)
					{
						xValue = (float)Math.Round(xValue, 2);
					}
					if (old.y != yValue)
					{
						yValue = (float)Math.Round(yValue, 2);
					}
				...
				if (property.propertyType == SerializedPropertyType.Vector2)
				{
					...
					MinMaxSlider( ref sliderValue.x, sliderRect, ref sliderValue.y);
				...
				else if (property.propertyType == SerializedPropertyType.Vector2Int)
				{
					...
					MinMaxSlider( ref xValue, sliderRect, ref yValue);
